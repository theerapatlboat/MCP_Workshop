...
คู่มือการติดตั้งและใช้งานระบบ AI Chatbot บน Facebook Messenger ที่เชื่อมต่อกับ GoSaaS Order Management
...
13. [Deploy ขึ้น Production](#13-deploy-ขึ้น-production)
...
ระบบนี้ประกอบด้วย 3 ส่วนหลักที่ทำงานร่วมกัน ทำให้ลูกค้าสามารถสั่งซื้อสินค้า ติดตามพัสดุ ดูรายงานยอดขาย และอื่นๆ ผ่าน Facebook Messenger ได้โดยตรง:

```
ลูกค้าพิมพ์ "สั่งสินค้า A 2 ชิ้น" ผ่าน Messenger
        ↓
   Facebook Platform
        ↓ POST event
┌─────────────────────────────────────────┐
...
┌─────────────────────────────────────────┐
│  Agent API (port 8002)                  │
│  - เก็บประวัติสนทนาแยกตาม sender_id     │
...
| **Agent API** | 8002 | `agent/agent_api.py` | AI Agent ที่ประมวลผลข้อความและตัดสินใจเรียก tools |
...
```bash
# ติดตั้ง dependencies สำหรับทั้งโปรเจค
pip install fastapi uvicorn httpx python-dotenv openai-agents fastmcp pydantic

# หรือติดตั้งเฉพาะ webhook
cd webhook
...
ไฟล์นี้อยู่ที่ root ของโปรเจค ใช้โดย MCP Server และ Agent API:
...
| ตัวแปร | คำอธิบาย |
|--------|----------|
| `OPENAI_API_KEY` | API key จาก OpenAI สำหรับ GPT model |
| `UAT_API_KEY` | API key สำหรับเรียก GoSaaS API |
| `UAT_API_URL` | Base URL ของ GoSaaS API |
| `MCP_SERVER_URL` | URL ของ MCP Server (default: `http://localhost:8000/mcp`) |
| `AGENT_MODEL` | model ที่ Agent ใช้ (default: `gpt-4o-mini`) |
...
| `AI_AGENT_URL` | URL ของ Agent API | `http://localhost:8002/chat` |
...
ต้องเปิด **3 terminal** พร้อมกัน ตามลำดับ:
...
รอจนเห็น:
```
Agent API ready — 16 tools loaded, model: gpt-4o-mini
```
...
http://localhost:8002  →  Agent API (AI)
```
...
curl -X POST http://localhost:8002/chat ^
  ...
  -d "{\"sender_id\": \"test_user\", \"message\": \"สวัสดี\"}"
```

ผลลัพธ์: `{"reply": "สวัสดีครับ! ยินดีให้บริการ..."}`
...
> หมายเหตุ: ทดสอบ local ด้วย curl จะไม่มี X-Hub-Signature-256 header แต่ระบบจะข้ามการตรวจ signature ถ้า `FB_APP_SECRET` มีค่า ให้เคลียร์ค่าชั่วคราวหรือเพิ่ม header ที่ถูกต้อง
...
│   └── agent_api.py              # API version — port 8002 (สำหรับ webhook)
│
...
4. ถ้าเป็น text message → `POST http://localhost:8002/chat` พร้อม `sender_id` + `message`
...
| Endpoint | Method | คำอธิบาย |
|----------|--------|----------|
| `/chat` | POST | รับ `sender_id` + `message` → ให้ AI ประมวลผล → ตอบ `reply` |
| `/chat/clear` | POST | ล้างประวัติสนทนาของ sender_id นั้น |

**ฟีเจอร์:**

- **Conversation history** — เก็บประวัติสนทนาแยกตาม `sender_id` ในหน่วยความจำ ทำให้คุยต่อเนื่องได้
- **OpenAI Agents SDK** — ใช้ `Runner.run()` ที่ตัดสินใจเองว่าจะเรียก tool ไหน
- **MCP connection** — เชื่อมต่อ MCP Server ตอน startup และปิดตอน shutdown อัตโนมัติ
...
// Request
POST /chat
{"sender_id": "12345", "message": "ค้นหาสินค้าเสื้อยืด"}

// Response
{"reply": "พบสินค้าเสื้อยืด 3 รายการ:\n1. เสื้อยืดคอกลม - ฿250\n2. ..."}
```
...
Agent ตัวเดียวกันแต่รันเป็น **interactive CLI** สำหรับทดสอบใน terminal:
...
```
You: ยอดขายวันนี้เท่าไร
Assistant: ยอดขายวันนี้มีทั้งหมด 15 ออเดอร์ รวม ฿12,500...
```

มี ConsoleTraceProcessor แสดง trace แบบ real-time พร้อมสีเพื่อ debug
